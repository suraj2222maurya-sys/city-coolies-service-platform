...
function TankServiceCard({ service }: { service: WaterTankCleaningService }) {
  ...
  const capacity =
    service.capacities.find((item) => item.id === capacityId) ??
  ...
  return (
    <article
      className={`cc-tank-card${
        service.featured ? " cc-tank-card--featured" : ""
      }`}
    >
      <div className="cc-tank-card__media">
        ...
        />
        {service.featured && (
          <span className="cc-tank-card__badge">Most booked</span>
        )}
      </div>
      ...
      <div className="cc-tank-card__body">
        <div className="cc-tank-card__topline">
          <span>
            ★ {service.rating} ({service.reviewCount.toLocaleString("en-IN")})
          </span>
            ...
            >
              {service.capacities.map((item) => (
                <option value={item.id} key={item.id}>
                  {item.label} — ₹{WATER_TANK_RATE_PER_LITRE.toFixed(2)} / litre
                </option>
              ))}
            </select>
          </label>
        ...
        <div className="cc-tank-card__estimate">
          <span>
            ...
            <strong>₹{WATER_TANK_RATE_PER_LITRE.toFixed(2)}</strong>
          </span>
        ...
        <p className="cc-tank-card__inspection">
          <strong>Pricing:</strong> Water tank cleaning is charged at ₹0.15 per
          litre for every service. The total is calculated from the selected
        ...
        {bookingItems.length > 0 ? (
          <ServiceBookingModal
            ...
            serviceName={`${service.name} — ${capacity?.label ?? ""} — ${quantity} tank${
              quantity > 1 ? "s" : ""
            }`}
            originalPrice={total}
            offerPrice={total}
    ...
    >
      <div className="cc-water-tank__container">
        <header className="cc-water-tank__header">
          ...
          <h2 id="cc-water-tank-title">Professional water tank cleaning</h2>
          <span>
        ...
        <div className="cc-water-tank__grid">
          {WATER_TANK_CLEANING_SERVICES.map((service) => (
            <TankServiceCard service={service} key={service.id} />
          ))}
        </div>
...
path = Path("/mnt/data/WaterTankCleaningSection.PRICE_ONLY_FIX.tsx")
